...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    // `fetchUser` hace `setState` recién después del `await`, pero el
    // linter de `set-state-in-effect` no distingue eso y lo marca igual
    // — es el fetch-on-mount documentado por React, con guard explícito
    // se cae en el mismo falso positivo.
