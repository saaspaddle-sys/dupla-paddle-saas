...
El scaffold `app.controller.ts`/`app.service.ts` (y sus specs) ya no existe — se borró cuando entró el primer módulo real, siguiendo la regla de abajo. `AppModule` importa `ConfigModule` (global, carga el `.env` de la raíz del monorepo), `ThrottlerModule` (global, sin registrarse como `APP_GUARD` — `GET /health` no se throttlea), `PrismaModule`, `HealthModule` (`GET /health` — ver `docs/database.md`, y la entrada del 2026-08-16 en `docs/decisions.md`), `PlayersModule`, `AuthModule`, `ClubsModule`, `TournamentsModule` y `SubscriptionsModule`, y registra `ValidationPipe` y el filtro de excepciones globales como providers (`APP_PIPE`/`APP_FILTER`, no en `main.ts` — ver por qué en la próxima sección). `main.ts` monta Swagger antes de `listen()` (ver abajo). El schema vigente también incluye la persistencia de billing (`SubscriptionCheckout`, `SubscriptionPreapprovalTombstone` y `BillingJobLock`). La llave, la carga de resultados y el avance automático ya tienen endpoints autenticados.
...
La integración de Mercado Pago vive en `src/subscriptions/`: checkout recurrente durable, webhook firmado, consulta canónica del cobro, activación idempotente, cancelación y reconciliación. La guía completa está en `docs/mercado-pago-integration.md`.
