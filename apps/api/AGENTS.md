...
- **La carga de resultados y el avance por partidos jugados.** `matches` y `match_sets` ya existen en el schema y sus migraciones. Faltan los endpoints para registrar resultados, propagar sus ganadores y finalizar el torneo. La generación de la llave ya escribe `in_progress` y resuelve los byes con su avance automático; su eliminación vuelve el torneo a `open` si no tiene resultados ni sets cargados (los byes no impiden borrarla). El `PATCH` del torneo sigue permitiendo solo la transición a `canceled`: pedir `in_progress` o `finished` devuelve `409 invalid_status_transition`.
...
- **La integración con Mercado Pago.** Ojo con esta: la tabla `payment_events` **está migrada pero no la toca ni una línea de código**. Es la excepción consciente que documenta `docs/data-model.md`. Ver un modelo de Prisma no significa que exista un webhook, un checkout ni validación de firma. La integración debe formar parte del release inicial y reemplaza cualquier activación manual, pero todavía está sin escribir.
...
Y lo que dejó de faltar con el slice de torneos (`tournaments` + `teams`): **el guard de tenancy por fin filtra algo real**. `TournamentsModule` (`src/tournaments/`) sirve once endpoints de la clase `club`: cuatro de torneos, cuatro de duplas (incluido `PATCH /tournaments/:tournamentId/teams/:teamId` para asignar o quitar `seed`) y tres de la llave (`POST`, `GET` y `DELETE /tournaments/:tournamentId/bracket`). `TournamentsController`/`TournamentsService`, `TeamsController`/`TeamsService` y `BracketController`/`BracketService` están registrados en ese módulo; el generador puro vive en `src/matches/bracket.builder.ts`. Tres piezas que conviene conocer antes de completar el slice 4:

- **`runSerializable`** (`src/common/prisma/serializable.ts`) corre un handler en una transacción `Serializable` y reintenta de forma **acotada** (3 intentos) ante `P2034`. Es la herramienta para cualquier read-modify-write donde no haya un índice único que sirva de red — hoy la cuota de torneos, el alta de duplas y la generación y eliminación de la llave. El handler tiene que ser idempotente porque puede correr más de una vez.
- **La FK compuesta de tenancy.** `teams(tournament_id, club_id)` referencia `tournaments(id, club_id)`, con un `@@unique([id, clubId])` en `Tournament` como blanco. `teams.club_id` es una copia denormalizada y esa FK es lo que impide que mienta. `matches(tournament_id, club_id)` ya repite ese patrón hacia `tournaments`, y `match_sets(match_id, club_id)` hacia `matches`.
...
Detalle del slice de torneos en `docs/decisions.md`, las entradas del 2026-09-02 y del 2026-09-03. El estado actual de la llave se verifica en `src/tournaments/bracket.controller.ts`, `src/tournaments/bracket.service.ts` y las migraciones `20260904161434_add_matches_and_match_sets` y `20260904180124_add_team_seed`.
...
- **El documento se commitea en `apps/api/openapi.json`** — es lo que lee `apps/web` sin levantar nada. Lo emite `src/swagger/generate-openapi.ts` (`pnpm run openapi`), y `pnpm run openapi:check` falla si quedó desincronizado; CI lo corre después del build. Si tocaste un controller o un DTO de respuesta, regeneralo y commitealo en el mismo PR. El script bootea `AppModule` con `preview: true`: arma el grafo de módulos sin instanciar providers, así que no pide `DATABASE_URL` ni abre conexión. Corre sobre `dist/` por lo del plugin, que es el punto de abajo.
