...
El scaffold `app.controller.ts`/`app.service.ts` (y sus specs) ya no existe — se borró cuando entró el primer módulo real, siguiendo la regla de abajo. `AppModule` importa `ConfigModule` (global, carga el `.env` de la raíz del monorepo), `ThrottlerModule` (global, sin registrarse como `APP_GUARD` — `GET /health` no se throttlea), `PrismaModule`, `HealthModule` (`GET /health` — ver `docs/database.md`, y la entrada del 2026-08-16 en `docs/decisions.md`), `PlayersModule`, `AuthModule` y `ClubsModule`, y registra `ValidationPipe` y el filtro de excepciones globales como providers (`APP_PIPE`/`APP_FILTER`, no en `main.ts` — ver por qué en la próxima sección). `main.ts` monta Swagger antes de `listen()` (ver abajo). Hay cuatro entidades migradas: `User` (`users`), `Player` (`players`), `Club` (`clubs`) y `Subscription` (`subscriptions`); el resto de `docs/decisions.md` sigue sin implementar.
...
- **Entidades con `club_id`.** El guard de tenancy ya está cableado (ver abajo), pero todavía no hay ninguna tabla que lo consuma: `clubs` es la raíz del árbol y no lleva `club_id`, `players` es global y no lo lleva por diseño. La primera que lo va a llevar es `tournaments`, en el slice 3.
