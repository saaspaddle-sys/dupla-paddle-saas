...
Queda derogada la regla de "rutas en español" de `docs/api-conventions.md` — con ella, la excepción de `/auth/*` deja de tener sentido y desaparece. `POST /auth/registro` pasa a `POST /auth/register` antes de que exista un solo consumidor, que es el único momento barato para hacerlo.
