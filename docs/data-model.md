...
| Release inicial · billing | `payment_events`, `subscription_checkouts`, `subscription_preapproval_tombstones`, `billing_job_locks` ✅ | 2          | upgrade `free → basic`/`pro` con Mercado Pago            |
...
- **`payment_events` fue la única tabla migrada por adelantado.** Esa excepción histórica permitió fijar la identidad idempotente del evento sin adivinar el resto del billing. La integración ya está implementada: los checkouts, tombstones, locks y columnas del mandato entraron junto con el código que los usa.
...
- **`subscriptions`** — suscripción del usuario dueño (1:1). El plan define cuotas (p. ej. `max_tournaments`). Arranca en **`free` y `active`**: una llave activa, suficiente para correr una categoría entera y descubrir el producto. `basic` (3) y `pro` (12) son pagos y se activan exclusivamente tras la confirmación de Mercado Pago; `pending` significa una suscripción paga esperando esa confirmación. No existe activación manual. Ver la entrada del 2026-09-09 en [decisions.md](./decisions.md).
- **`subscription_checkouts`** — reserva durable de cada intento de upgrade. Congela referencia, plan, importe y moneda antes de llamar al proveedor; `recovery_required` evita crear un segundo mandato cuando el resultado externo es ambiguo.
- **`payment_events`** — bitácora operativa que hace idempotente al webhook. El evento se persiste antes de aplicar el cobro; `processed_at` distingue lo resuelto y `last_error_at` deja reintentables las fallas transitorias. Los índices únicos protegen tanto la identidad de la notificación como la del recurso cobrado.
- **`subscription_preapproval_tombstones`** — conserva la identidad de mandatos cancelados para que una factura tardía se audite sin reactivar una suscripción nueva.
- **`billing_job_locks`** — lease distribuido del reconciliador; evita que varias instancias procesen simultáneamente el mismo lote de eventos pendientes y vencimientos.
