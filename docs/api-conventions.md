...
- **`/tournaments/:id/bracket` va en singular**, y es la tercera excepción reconocida. Un torneo tiene exactamente un cuadro: `/brackets` prometería una colección que nunca va a tener dos elementos, y dejaría al `DELETE` sin destino salvo inventando un id que el cliente no necesita para nada. Es el mismo criterio que hace canónico a `me`. La regla general no cambia: la excepción es para un **sub-recurso singleton**, o sea uno del que la relación padre-hijo ya garantiza que hay a lo sumo uno. Un sub-recurso del que puede haber varios va en plural aunque hoy tenga uno solo.
...
- **`@ApiTags` con el dominio de negocio**, tomado de `SWAGGER_TAGS` (`Auth`, `Clubs`, `Players`, `Tournaments`, `Matches`, `Operations`), nunca un string suelto. La UI conserva ese orden explícito, no el alfabético. Por ejemplo, las duplas y el cuadro van bajo `Tournaments`, y la carga de resultados va bajo `Matches`.
- **La clase de endpoint es un contrato separado de su tag.** `club`, `public`, `platform` y `ops` siguen definiendo auth, tenancy, audiencia y restricciones de datos; no se infieren desde el agrupamiento visual de Swagger. Un endpoint de torneos del club, por ejemplo, se documenta bajo `Tournaments` y mantiene sus guards y su `@ApiBearerAuth` por ser clase `club`.
